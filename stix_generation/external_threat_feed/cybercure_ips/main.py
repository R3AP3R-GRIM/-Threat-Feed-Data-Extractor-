        ...
        if response.status_code == 200:
            ...
            for line in response.content.decode("utf-8").split(","):
                ...
                if re.match(ipv4_pattern, line):
                    ioc_list.append(convert_to_stix(line, feed_name, feed_source, url, "ipv4-addr"))
                elif re.match(ipv6_pattern, line):
                    ioc_list.append(convert_to_stix(line, feed_name, feed_source, url, "ipv6-addr"))
                elif re.match(md5_pattern, line):
                    ioc_list.append(convert_to_stix(line, feed_name, feed_source, url, "file:hashes.MD5"))
                elif re.match(sha1_pattern, line):
                    ioc_list.append(convert_to_stix(line, feed_name, feed_source, url, "file:hashes.SHA-1"))
                elif re.match(sha256_pattern, line):
                    ioc_list.append(convert_to_stix(line, feed_name, feed_source, url, "file:hashes.SHA-256"))
                elif re.match(sha512_pattern, line):
                    ioc_list.append(convert_to_stix(line, feed_name, feed_source, url, "file:hashes.SHA-512"))
                elif re.match(url_pattern, line):
                    ioc_list.append(convert_to_stix(line, feed_name, feed_source, url, "url"))
...
def main():
    ...
    if os.path.exists(config_file_path):
        ...
        url = config['URL']['cybercure_ips']
        feed_name = 'cybercure_ips'
